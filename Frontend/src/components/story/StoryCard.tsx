...
import { Clock, ChevronRight, Trash2, BookOpen, GitBranch } from 'lucide-react'
  ...
  return (
    <Card hoverable className="h-full">
      <div className="flex flex-col h-full">
        ...
        {/* Completed Story Actions */}
        {story.is_completed && (
          <div className="flex items-center gap-2 mb-4">
            <Link to={`/story/${story.id}/read`} className="flex-1">
              <Button 
                variant="secondary" 
                size="sm" 
                className="w-full"
            ...
            <Link to={`/story/${story.id}/tree`} className="flex-1">
              <Button 
                variant="ghost" 
                size="sm" 
                className="w-full"
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-story-border">
          ...
          <Link to={`/story/${story.id}`}>
            <motion.div 
              ...
              whileHover={{ x: 4 }}
            >
              {story.is_completed ? 'Continue' : 'Continue'}
